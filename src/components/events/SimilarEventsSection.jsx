...
import { useSimilarEvents } from '@/hooks/useEventRecommendations';
...
const SimilarEventsSection = ({ eventId, limit = 6, showViewAll = true }) => {
  const { data: events = [], isLoading } = useSimilarEvents(eventId, limit);
  ...
  return (
    <Container>
      ...
      {events.length === 0 ? (
        <EmptyState>
          No similar events found. Check out other events!
        </EmptyState>
      ) : (
        <EventsList>
          {events.map(event => (
